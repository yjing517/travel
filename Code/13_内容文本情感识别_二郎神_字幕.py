...
def audio2emo_text(aid_df):
    ...
    if os.path.exists(f"./Result/text2emo_srt.csv"):
        text2emo_text_df = pd.read_csv(f"./Result/text2emo_srt.csv",encoding='utf-8')
    else:
        text2emo_text_df = pd.DataFrame(columns=['aid','text_emo_binary'])
        text2emo_text_df.to_csv(f"./Result/text2emo_srt.csv", index=False,encoding='utf-8')
    ...
    for index, row in aid_df.iterrows():
        ...
        # 未完成文本情感识别
            ...
            if type(text_list) == str:
                ...
                with open(f"./Result/text2emo_srt.csv", 'a+', newline='',encoding='utf-8') as file:
